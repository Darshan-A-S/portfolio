...
import { checkpointRailDoc as doc } from "../data/checkpoint-rail-doc"
...
function DemoPreview({ children }) {
  return (
    <div className="mb-3 flex min-h-[380px] items-center justify-center rounded-lg border border-[color:var(--color-border)] bg-[var(--color-bg-secondary)] p-6">
      {children}
    </div>
  )
}
...
export default function CheckpointRailDoc() {
    ...
    <article className="flex flex-1 flex-col px-[8px] sm:px-0">
      <div className="border-b border-[color:var(--color-border)]">
        <div className="mx-auto flex max-w-[768px] items-center justify-between gap-4 border-x border-[color:var(--color-border)] px-4 py-2">
          ...
          <ComponentNav slug="checkpoint-rail" />
        </div>
      </div>

      <div className="border-b border-[color:var(--color-border)]">
        <div className="mx-auto max-w-[768px] border-x border-[color:var(--color-border)] px-4 py-6">
          <h1 className="text-[26px] font-bold leading-snug text-[var(--color-text)]">Checkpoint Rail</h1>
          <p className="mt-2 text-[14px] leading-relaxed text-[var(--color-text-muted)]">
            Nothing theme chat navigation rail. Published in <Mono>dasregistry</Mono> (view this on desktop to see the component in action).
          </p>
        </div>
      </div>
      ...
      <div className="mx-auto flex w-full max-w-[768px] flex-1 flex-col border-x border-b border-[color:var(--color-border)]">
        <Section title="Demo">
          ...
          <CodeBlock title="checkpoint-rail-demo.tsx" code={doc.demoCode} expandable />
        </Section>
        ...
        <Section title="Installation">
          ...
          {tab === "cli" ? (
            <div className="flex flex-col gap-4">
              <CodeBlockCommand prompt="Install the CheckpointRail component" {...convertNpmCommand("npm install dasregistry")} />
              <p className="text-[13px] text-[var(--color-text-muted)]">
                Import the stylesheet (Nothing theme: black screen, dot-matrix font, red accents, adapts to light/dark mode):
              </p>
            ...
            <ol className="space-y-6">
              ...
              <li>
                ...
                <CodeBlock title="components/checkpoint-rail.tsx" code={doc.sourceCode} expandable />
              </li>
              <li>
        ...
        <Section title="API reference">
          <h3 className="mb-3 text-[14px] font-bold text-[var(--color-text)]">CheckpointRailProps</h3>
